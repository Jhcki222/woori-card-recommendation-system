...
import TiltedCard from '../bits/TitledCard/TitledCard';
...
export default function Card({ card, index }: CardProps) {
    ...
    const benefitsArray = card.benefits ? card.benefits.split(',').map((b) => b.trim()) : [];
    ...
    return (
        ...
        >
            <TiltedCard
                ...
                containerHeight="300px" // h-64
                imageWidth="100%"
                imageHeight="93%"
                scaleOnHover={1.1}
                rotateAmplitude={15}
                showTooltip={true}
                displayOverlayContent={true} // 오버레이는 사용하지 않습니다.
            />
            ...
            <div className="p-3 flex-1 flex flex-col">
                <div className="flex items-center gap-2 mb-2">
                    ...
                    <h3 className="text-sm font-bold text-white line-clamp-1">{card.cardName}</h3>
                </div>
                <ul className="space-y-1 text-xs text-blue-100 mb-3 flex-1">
                    {benefitsArray.slice(0, 2).map((benefit, benefitIndex) => (
                        <li
                            ...
                            style={{
                                animationDelay: `${index * 150 + benefitIndex * 50}ms`,
                            }}
                        >
                            <CheckIcon />
